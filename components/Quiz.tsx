...
const Quiz: React.FC<QuizProps> = ({ settings, userData, onQuit, onUpdatePerformance, onDrillComplete, onToggleSkipPreDrillInfo, isQuietMode = false }) => {
  ...
  const { playNote, stopNote } = useSustainedNote(settings.instrument, isQuietMode);
  ...
  const playTone = useCallback((freq: number, duration: number, type: OscillatorType = 'sine', volume: number = 0.2) => {
    // This function will be more complex with proper audio context management
    // For now, this is a placeholder to show where sound would be played.
  }, [isQuietMode]);
  ...
  const playLossSound = useCallback(() => {
    // Placeholder for loss sound
  }, [playTone]);

  const playCorrectSound = useCallback(() => {
    // Placeholder for correct sound
  }, [playTone]);

  const playIncorrectSound = useCallback(() => {
    // Placeholder for incorrect sound
  }, [playTone]);

  const playTickSound = useCallback(() => {
    // Placeholder for tick sound
  }, [playTone]);
  ...
  const handleRootKeySelection = useCallback((selectedKey: MusicKey) => {
    ...
    const handleScaleSweeperAnswer = useCallback((playedUniqueNote: string, playedNoteName: Note) => {
        ...
        const isCorrect = scale.notes.some(note => note === playedNoteName);
        onUpdatePerformance({ ...currentQuestion, isCorrect, drillMode: settings.drillMode });
        ...
        if (isCorrect) {
            ...
            if (playedUniqueNote === finalOctaveNoteId) {
                ...
                const uniqueFoundNoteNames = new Set(newFoundNotes.map(n => n.replace(/-?\d.*$/, '') as Note));
    ...
    const handleNoteProfessorAnswer = useCallback(async (playedUniqueNote: string, playedNoteName: Note) => {
      if (!currentQuestion) return;
    ...
    const handleNoteDiscoveryChoice = useCallback(async (choice: Note) => {
        if (!currentQuestion) return;

        await playNoteSound(`${choice}4`, 'Piano', isQuietMode);

        setTotalResponseTime(t => t + (Date.now() - questionStartTime));
        const isCorrect = choice === (currentQuestion.correctAnswers[0].replace(/-?\d.*$/, '') as Note);
    ...
    }, [currentQuestion, onUpdatePerformance, playCorrectSound, playIncorrectSound, goToNextQuestion, discoveryRound, revealedNoteNames, handleFinish, questionStartTime, streak, consecutiveErrors, handleNoteDiscoveryFailure, isQuietMode]);

  const handleAnswer = useCallback(async (playedUniqueNote: string, event?: React.MouseEvent) => {
    if ((gameState !== 'playing' && sweeperPhaseRef.current !== 'intermission') || quizPhase !== 'active' || !currentQuestion) return;
    
    const noteMatch = playedUniqueNote.match(/^[A-G][b#]?/);
    if (!noteMatch) {
      console.error(`Invalid note name parsed from played unique note: ${playedUniqueNote}`);
      return;
    }
    
    const getEnharmonicEquivalent = (note: string): Note => {
        const equivalents: Record<string, Note> = {
            'C#': 'Db', 'D#': 'Eb', 'G#': 'Ab', 'A#': 'Bb',
            'B#': 'C', 'E#': 'F', 'Gb': 'F#', 'Cb': 'B', 'Fb': 'E',
        };
        const mappedNote = equivalents[note];
        if (mappedNote) {
            return mappedNote;
        }
        if ((ALL_NOTES as ReadonlyArray<string>).includes(note)) {
            return note as Note;
        }
        console.warn(`Could not resolve note: ${note}. Defaulting to 'C'.`);
        return 'C';
    };
    const playedNoteName = getEnharmonicEquivalent(noteMatch[0]);

    if (isAnyDegreeDash) return;
    
    await playNote(playedUniqueNote);

    if (isNoteProfessorMode) await handleNoteProfessorAnswer(playedUniqueNote, playedNoteName);
    else if (isGalaxyConstructorMode) return;
  ...
  const renderedInstrument = useMemo(() => {
    ...
    const isDisabled = isNoteDiscoveryMode || isGalaxyConstructorMode || isAnyDegreeDash;
    ...
    const pianoRange = { startMidi: 36, keyCount: 61 };
    
    switch (settings.instrument) {
      case 'Piano': return <Piano {...commonProps} range={pianoRange} />;
      case 'Guitar':
      ...
      isNoteProfessorMode, isNoteDiscoveryMode, highlightedUniqueId, revealedNoteNames, quizPhase, animatingSacrifice, isAnyDegreeDash
  ]);
