...
import { DrillSettings, TuningPreset, Language } from '../types';
...
const Tuner: React.FC<TunerProps> = ({ settings, onSettingChange, language }) => {
  ...
  const renderStatusMessage = () => {
    switch(status) {
        ...
        case 'denied':
            return <p className="text-red-400 text-sm">{t('micPermissionDenied')}</p>;
        case 'error':
        ...
        case 'unavailable':
            return <p className="text-red-400 text-sm">{t('micNotSupported')}</p>;
        case 'idle':
        default:
  ...
  return (
    <div className="bg-stone-900/70 backdrop-blur-lg border border-stone-700/50 p-4 sm:p-6 rounded-xl shadow-2xl w-full max-w-lg mx-auto flex flex-col gap-4">
        <header className="text-center">
            <h2 className="text-3xl font-bold text-orange-400">{t('tuner')}</h2>
        ...
        <main className="flex flex-col items-center justify-center gap-6 p-4 bg-black/30 rounded-lg min-h-[300px]">
            {isMicReady ? (
                <>
                    <TuningDisplay tunerResult={tunerResult} isAudible={isAudible} language={language} />
