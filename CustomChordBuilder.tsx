...
import { Note, ChordTypeName, Language } from '../types';
...
const CustomChordBuilder: React.FC<CustomChordBuilderProps> = ({ name, notes, onNameChange, onSave, onClear, onCancel, detectedChord, language }) => {
  ...
  return (
    <div className="bg-black/30 rounded-lg p-3 sm:p-4 animate-key-appear border border-green-500/50">
      <div className="flex justify-between items-baseline mb-2">
        {/* FIX: Changed incorrect translation key from 'chordBuilder' to 'chordBuilderTitle'. */}
        <h3 className="text-lg font-semibold text-green-400">{t('chordBuilderTitle')}</h3>
        {detectedName && (
      ...
      <div className="space-y-3">
        ...
        <div className="bg-stone-800/50 rounded-md p-2 min-h-[40px] text-stone-300 text-sm">
          {t('notes')}{' '}
          <span className="font-semibold text-white">{noteDisplay || t('notesPlaceholder')}</span>
        </div>
