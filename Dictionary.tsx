...
import { UserData, Note, ChordTypeName, InstanceState, VoicingType, Chord, Language } from '../types';
...
const ChordSelectorModal: React.FC<ChordSelectorModalProps> = ({ isOpen, onClose, onSelectChord, language }) => {
  ...
  return (
    <div className="w-full h-full flex flex-col text-white p-2">
      ...
      <header className="flex-shrink-0 flex flex-wrap gap-2 items-center justify-between mb-4">
        <h2 className="text-2xl sm:text-3xl font-bold text-orange-400">{t('dictionaryTitle')}</h2>
        <button
      ...
      <main className="flex-1 flex flex-col min-h-0 bg-stone-800/50 rounded-lg shadow-lg overflow-hidden">
        {activeChord && fullChord ? (
            ...
            <div className="h-full flex flex-col items-center justify-center text-center text-stone-400">
                <h3 className="text-2xl font-bold text-stone-200">{t('dictionaryEmptyStateTitle')}</h3>
                <p className="mt-2 max-w-md">
                    {t('dictionaryEmptyStateText')}
                </p>
            </div>
        )}
      </main>
